...
// Modified for dbus-psi-genset (Venus OS v3.5x / gui-v2):
//  - adds a "DC input" measurement group (/Dc/0/Voltage|Current|Power)
//  - adds an editable "DC cutoff thresholds" sub-page (/Settings/LVD|LVDR|HVDR|HVD)
// All additions are gated on the relevant paths being valid, so the stock page
// is unaffected for other (non-PSI) gensets. Based on the upstream v3.50 file.
...
ObjectModel {
	...
	ListQuantityGroup {
		text: "DC input"
		allowed: defaultAllowed && psiDcVoltage.isValid
		textModel: [
			{ value: psiDcVoltage.value, unit: VenusOS.Units_Volt_DC },
			{ value: psiDcCurrent.value, unit: VenusOS.Units_Amp, visible: psiDcCurrent.isValid },
			{ value: psiDcPower.value, unit: VenusOS.Units_Watt, visible: psiDcPower.isValid },
		]

		VeQuickItem { id: psiDcVoltage; uid: root.bindPrefix + "/Dc/0/Voltage" }
		VeQuickItem { id: psiDcCurrent; uid: root.bindPrefix + "/Dc/0/Current" }
		VeQuickItem { id: psiDcPower; uid: root.bindPrefix + "/Dc/0/Power" }
	}
	...
	ListNavigationItem {
		text: "DC cutoff thresholds"
		allowed: defaultAllowed && (psiLvd.isValid || psiLvdr.isValid || psiHvdr.isValid || psiHvd.isValid)
		onClicked: Global.pageManager.pushPage(psiCutoffComponent, {"title": text})

		VeQuickItem { id: psiLvd; uid: root.bindPrefix + "/Settings/LVD" }
		VeQuickItem { id: psiLvdr; uid: root.bindPrefix + "/Settings/LVDR" }
		VeQuickItem { id: psiHvdr; uid: root.bindPrefix + "/Settings/HVDR" }
		VeQuickItem { id: psiHvd; uid: root.bindPrefix + "/Settings/HVD" }

		Component {
			id: psiCutoffComponent

			Page {
				GradientListView {
					model: ObjectModel {
						ListSpinBox {
							text: "Low voltage disconnect (LVD)"
							dataItem.uid: root.bindPrefix + "/Settings/LVD"
							decimals: 1
							stepSize: 0.1
							from: 5
							to: 35
						...
						ListSpinBox {
							text: "Low voltage reconnect (LVDR)"
							dataItem.uid: root.bindPrefix + "/Settings/LVDR"
							decimals: 1
							stepSize: 0.1
							from: 5
							to: 35
						...
						ListSpinBox {
							text: "High voltage reconnect (HVDR)"
							dataItem.uid: root.bindPrefix + "/Settings/HVDR"
							decimals: 1
							stepSize: 0.1
							from: 5
							to: 35
						...
						ListSpinBox {
							text: "High voltage disconnect (HVD)"
							dataItem.uid: root.bindPrefix + "/Settings/HVD"
							decimals: 1
							stepSize: 0.1
							from: 5
							to: 35
